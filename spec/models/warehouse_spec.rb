  ...
  describe '#valid?' do 
    ...
    context 'uniqueness' do 
      it 'false when code is already in use' do 
        warehouse = Warehouse.new(name: 'Aeroporto de SP', code: 'GRU', address: 'Avenida', zip_code: '20000-000', 
                                         city: 'São Paulo', area: 11000, description: 'Outra descrição')
        expect(warehouse.valid?).to eq false
      end
      it 'false when name is already in use' do 
        warehouse = Warehouse.new(name: 'Aeroporto SP', code: 'ASP', address: 'Avenida', zip_code: '20000-000', 
                                         city: 'São Paulo', area: 11000, description: 'Outra descrição')
        expect(warehouse.valid?).to eq false
      end
    end
